  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? 'border-transparent text-white' : 'border-border text-muted-foreground hover:border-foreground/30 hover:text-foreground',
      )}
...
const SEVERITY_OPTIONS: { value: HotspotSeverity | 'all'; label: string; color: string }[] = [
...
export function HotspotFilters({ filters, onChange, totalCount, filteredCount }: HotspotFiltersProps) {
    ...
    <div className="bg-card border border-border rounded-xl p-3">
      ...
      {expanded && (
        <div className="mt-3 pt-3 border-t border-border grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          {/* Source type */}
          <div>
            <p className="text-xs text-muted-foreground mb-1.5 font-medium">Source Type</p>
            <div className="flex flex-wrap gap-1.5">
              <ToggleChip
              ...
              {Object.entries(SOURCE_TYPE_CONFIG).map(([key, cfg]) => (
                <ToggleChip
                  ...
                  onClick={() => onChange({ ...filters, sourceType: key as PollutionSourceType })}
                  color={cfg.color}
                >
                  {cfg.emoji} {cfg.label}
                </ToggleChip>
